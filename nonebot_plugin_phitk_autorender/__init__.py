...
async def handle_render(bot: Bot, event: MessageEvent):
    ...
    try:
        ...
        chart_json, cover_png, song_mp3, info_text = await process_assemb(assemb_dir)
        logger.info("重命名完成")
...
async def process_assemb(assemb_dir: Path):
    ...
    os.system(f"cp {info_file} {assemb_dir}/info.yml -n")
    info_yml = assemb_dir / "info.yml"
    ...
    return assemb_dir / "chart.json", assemb_dir / "background.png", assemb_dir / "song.mp3", info_text
...
def parse_info(info_text: str) -> str:
    ...
    difficulty = data.get("Level", "Unknown Lv.?")
